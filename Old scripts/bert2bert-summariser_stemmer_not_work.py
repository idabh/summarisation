...
############################## Data ################################
#load through pandas
train = Dataset.from_pandas(pd.read_csv("./NLP/train_d.csv", usecols=['text','summary','idx']))
test = Dataset.from_pandas(pd.read_csv("./NLP/test_d.csv", usecols=['text','summary','idx']))
val = Dataset.from_pandas(pd.read_csv("./NLP/val_d.csv", usecols=['text','summary','idx']))
...
#tokenizer_tokens = BertTokenizer.from_pretrained("Maltehb/danish-bert-botxo")
...
train_data = dd['train'].select(range(110))
...
def compute_metrics(eval_pred):
    ...
    decoded_preds = tokenizer.convert_ids_to_tokens(predictions) # convert to tokens
    
    # Replace -100 in the labels as we can't decode them.
    labels = np.where(labels != -100, labels, tokenizer.pad_token_id)
    decoded_labels = tokenizer.convert_ids_to_tokens(labels)
    ...
    for word in decoded_preds:
        stemmed_preds.append(stemmer.stem(word))
    for word in decoded_labels:
       stemmed_labels.append(stemmer.stem(word))

    # back to ids
    stemmed_preds =  tokenizer.convert_tokens_to_ids(stemmed_preds)
    stemmed_labels =  tokenizer.convert_tokens_to_ids(stemmed_labels)

    stemmed_preds = tokenizer.batch_decode(stemmed_preds, skip_special_tokens=True)
    stemmed_labels = tokenizer.batch_decode(stemmed_labels, skip_special_tokens=True)

    print(stemmed_preds, stemmed_labels)
    result = metric.compute(predictions=stemmed_preds, references=stemmed_labels)
